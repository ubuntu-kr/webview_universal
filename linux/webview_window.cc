...
void WebviewWindow::EvaluateJavaScript(const char *java_script, FlMethodCall *call) {
  webkit_web_view_evaluate_javascript(
      ...
      [](GObject *object, GAsyncResult *result, gpointer user_data) {
        ...
        JSCValue *js_result = webkit_web_view_evaluate_javascript_finish(WEBKIT_WEB_VIEW(object), result, &error);
        if (!js_result) {
        ...
        if (exception) {
          fl_method_call_respond_error(call, "failed to evaluate javascript.", jsc_exception_get_message (exception), nullptr, nullptr);
        }else{
